...
function Builder() {
  ...
  if(process.platform == 'win32') {
    if(!process.env["VCINSTALLDIR"] && !process.env["VS100COMNTOOLS"]) {
      console.log(ansi.yellow("You appear to not be running in a Visual Studio prompt. Using MinGW instead."));
      this.cppCompiler = "./MinGW/bin/g++.exe";
      this.linker = "./MinGW/bin/g++.exe";
    } else {
...
Builder.prototype.link = function(callback) {
  ...
  this.createDir(this.ouputDir);
	...
	var objectFilesStr = this.objectFiles.map(function(f) { return path.relative(self.projectDir, f); }).join(' ');
