...
Builder.createBuilder = function(callback) {
  var builder = new Builder();
  builder.flagGroups = {};
  builder.target = "native_bindings";
  builder.sourceFiles = [];
  builder.verbose = false;
  builder.showWarnings = false;
  builder.cppCompiler = "g++";
  builder.linker = "g++";
  builder.objectFiles = [];

  if (process.platform == 'win32') {
    if (!process.env["VCINSTALLDIR"] && !process.env["VS100COMNTOOLS"]) {
      builder.fail("You appear to not be running in a Visual Studio prompt.");
    }
  }

  if (process.platform == 'win32') {
    builder.cppCompiler = "cl.exe";
    builder.linker = "link.exe";
    builder.nodeDir = process.env["NODE_HOME"];
  } else {
    builder.nodeDir = process.env["NODE_HOME"] || path.join(process.execPath, '..', '..');
  }

  if (builder.nodeDir) {
    builder.nodeDir = builder.trimQuotes(builder.nodeDir);
    builder.failIfNotExists(builder.nodeDir, 'Node path "%s" not found, try setting NODE_HOME');
    return gotNodeDir();
  } else {
    try {
      var gyp = require('node-gyp')();
      builder.nodeDir = path.resolve(gyp.devDir, process.version);
      console.log(builder.nodeDir);
      if (!existsSync(builder.nodeDir)) {
        builder.nodeDir = path.resolve(gyp.devDir, process.version.substr('v'.length));
      }

      if (existsSync(builder.nodeDir)) {
        return gotNodeDirViaGyp();
      } else {
        gyp.parseArgv(['node-gyp', 'install']);
        return gyp.commands.install([process.version], function(err, version) {
          return gotNodeDirViaGyp();
        });
      }
    } catch (e) {
      return builder.fail('Could not load node-gyp to download source. Either specify NODE_HOME or install node-gyp.', e);
    }
  }

  function gotNodeDirViaGyp() {
    builder.nodeDirViaGyp = true;
    var arch = process.arch || 'ia32';
    builder.nodeLibDir = path.resolve(builder.nodeDir, arch);
    gotNodeDir();
  }
  ...
  function gotNodeDir() {
    ...
    if (process.platform == 'win32') {
      builder.nodeIncludeDir = path.join(builder.nodeDir, 'src');
      builder.v8IncludeDir = path.join(builder.nodeDir, 'deps/v8/include');
      builder.uvIncludeDir = path.join(builder.nodeDir, 'deps/uv/include');
      builder.nodeLibDir = builder.nodeLibDir || path.join(builder.nodeDir, 'Release');
    } else {

      builder.nodeIncludeDir = path.join(builder.nodeDir, 'include', 'node');
      builder.nodeLibDir = builder.nodeLibDir || path.join(builder.nodeDir, 'lib');
    }
    builder.projectDir = path.resolve('.');
    builder.buildDir = path.resolve(builder.projectDir, 'build');
    builder.ouputDir = path.resolve(builder.buildDir, 'Release');

    builder.appendUnique('CXXFLAGS', '-c');

    if (process.platform == 'win32') {
      builder.appendUnique('CXXFLAGS', [
        '-nologo',
        '-DWIN32',
        '-D_WINDOWS',
        '-D_WINDLL',
      ...
      builder.appendUnique('LINKFLAGS', [
        '-nologo',
        '-dll',
        '-MANIFEST:NO',
        '-SUBSYSTEM:WINDOWS',
      ...
      builder.appendLinkerLibrary('node');
      if (!builder.nodeDirViaGyp) {
        builder.appendLinkerLibrary('uv');
      }
    } else {
      builder.appendUnique('CXXFLAGS', [
        '-D_LARGEFILE_SOURCE',
